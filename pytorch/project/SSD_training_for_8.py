...
from utils.ssd_model_for_8 import SSD
from utils.ssd_model_for_8 import make_datapath_list, VOCDataset, DataTransform, Anno_xml2list, od_collate_fn
from utils.ssd_model_for_8 import MultiBoxLoss
...
ssd_cfg = {
    ...
    'bbox_aspect_num': [6, 6,6, 6, 6, 6, 6],  # 출력할 DBox의 화면비의 종류
...
start_epoch = 10 
weight_file = f'./weights/11_ssd300_{start_epoch}.pth'
...
def train_model(net, dataloaders_dict, criterion, optimizer, start_epoch, end_epoch):
    ...
    for epoch in range(start_epoch,end_epoch+1):
        ...
        # 네트워크를 저장한다
        if ((epoch+1) % 10 == 0):
            torch.save(net.state_dict(), 'weights/11_ssd300_' +
                       str(epoch+1) + '.pth')
