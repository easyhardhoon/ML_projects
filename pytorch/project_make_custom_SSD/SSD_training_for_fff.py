...
from utils.ssd_model_for_fff import SSD
from utils.ssd_model_for_fff import make_datapath_list, VOCDataset, DataTransform, Anno_xml2list, od_collate_fn
from utils.ssd_model_for_fff import MultiBoxLoss
...
#    'num_classes': 21,  # 배경 클래스를 포함한 총 클래스 수
#    'input_size': 300,  # 이미지의 입력 크기
#    'bbox_aspect_num': [6, 6,6, 6, 6],  # 출력할 DBox의 화면비의 종류
#    'feature_maps': [38, 19, 19, 19, 1],  # 각 source의 이미지 크기
#    'steps': [8, 16, 16,  16, 300],
#    'min_sizes': [30, 60, 60,  60, 264],  # DBOX의 크기(최소)
#    'max_sizes': [60, 111,111,111, 315],  # DBOX의 크기(최대)
...
start_epoch = 10 
weight_file = f'./weights/fff_ssd300_{start_epoch}.pth'
...
def train_model(net, dataloaders_dict, criterion, optimizer, start_epoch, end_epoch):
    ...
    for epoch in range(start_epoch,end_epoch+1):
        ...
        # 네트워크를 저장한다
        if (epoch > 250):
            torch.save(net.state_dict(), 'weights/fff_ssd300_' +
                       str(epoch+1) + '.pth')
