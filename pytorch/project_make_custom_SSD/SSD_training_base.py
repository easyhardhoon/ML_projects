...
# SSD300 설정
#-----------------------------------------------------------------
# HOON : vanilla - "6" feature map
ssd_cfg = {
    'num_classes': 21,  # 배경 클래스를 포함한 총 클래스 수
    'input_size': 300,  # 이미지의 입력 크기
    'bbox_aspect_num': [4, 6, 6, 6, 4],  # 출력할 DBox의 화면비의 종류
    'feature_maps': [38, 19, 10, 5, 3],  # 각 source의 이미지 크기
    'steps': [8, 16, 32, 64, 100],
    'min_sizes': [30, 60, 111, 162, 213],  # DBOX의 크기(최소)
    'max_sizes': [60, 111, 162, 213, 264],  # DBOX의 크기(최대)
...
# SSD300 설정
# HOON : vailla - "6" feature map + change "5" feature map's bbox_aspect_num (4->6)
ssd_cfg = {
    'num_classes': 21,  # 배경 클래스를 포함한 총 클래스 수
    'input_size': 300,  # 이미지의 입력 크기
    'bbox_aspect_num': [4, 6, 6, 6, 6],  # 출력할 DBox의 화면비의 종류
    'feature_maps': [38, 19, 10, 5, 3],  # 각 source의 이미지 크기
    'steps': [8, 16, 32, 64, 100],
    'min_sizes': [30, 60, 111, 162, 213],  # DBOX의 크기(최소)
    'max_sizes': [60, 111, 162, 213, 264],  # DBOX의 크기(최대)
...
def train_model(net, dataloaders_dict, criterion, optimizer, start_epoch, end_epoch):
    ...
    for epoch in range(start_epoch,end_epoch+1):
        ...
        if (epoch > 159):
            if (epoch %10 ==0):
                torch.save(net.state_dict(), 'weights/hoon_ssd300_' + str(epoch+1) + '.pth')
