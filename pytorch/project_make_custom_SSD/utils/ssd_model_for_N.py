...
def make_vgg():
    ...
    #       512, 'MC', 1024, 1024, 1024, 'M', 1024, 1024, 1024]
...
def make_extras_X():
    layers = []
    in_channels = 1024  # # 34층에 걸쳐, vgg모듈을 작성
...
def make_extras():
    ...
    #cfg = [1024, 2048, 1024, 2048, 1024, 2048, 1024, 2048]
    
    #cfg = [1024, 2048, 1024, 2048, 1024, 2048, 1024, 2048]
    cfg = [128,128,128,128,128,128, 2048, 1024, 2048, 1024, 2048, 1024, 2048]

    layers += [nn.Conv2d(in_channels, cfg[0], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[0], cfg[1], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[1], cfg[2], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[2], cfg[3], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[3], cfg[4], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[4], cfg[5], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[5], cfg[6], kernel_size=(3), stride=2, padding=1)]
    layers += [nn.Conv2d(cfg[6], cfg[7], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[7], cfg[8], kernel_size=(3), stride=2, padding=1)]
    layers += [nn.Conv2d(cfg[8], cfg[9], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[9], cfg[10], kernel_size=(3))]
    layers += [nn.Conv2d(cfg[10], cfg[11], kernel_size=(1))]
    layers += [nn.Conv2d(cfg[11], cfg[12], kernel_size=(3))]
...
def make_loc_conf(num_classes=21, bbox_aspect_num=[6,6,6,6,6,4,4]):

    loc_layers = []
    conf_layers = []
    print(len(bbox_aspect_num))
    ...
    #-----------------------------------------------------------------------
    # VGG의 30층, conv5_3(source2)에 대한 합성곱층. nn.CONv2d(input data channel size..)
    # input data channel size  == each feature map's final conv layer's filter num
    # 이로 인해 이번 feature map 추가로직은 상대적으로 간단했으나
    # 더욱 다양하게 추가해볼 수 있다. (계획)
    ...
    loc_layers += [nn.Conv2d(2048, bbox_aspect_num[3]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(2048, bbox_aspect_num[3]
                              * num_classes, kernel_size=3, padding=1)]

    # extra(source5)에 대한 합성곱층
    loc_layers += [nn.Conv2d(2048, bbox_aspect_num[4]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(2048, bbox_aspect_num[4]
                              * num_classes, kernel_size=3, padding=1)]

    # extra(source6)에 대한 합성곱층
    loc_layers += [nn.Conv2d(2048, bbox_aspect_num[5]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(2048, bbox_aspect_num[5]
                              * num_classes, kernel_size=3, padding=1)]

    # extra(source7)에 대한 합성곱층
    loc_layers += [nn.Conv2d(2048, bbox_aspect_num[6]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(2048, bbox_aspect_num[6]
                              * num_classes, kernel_size=3, padding=1)]
...
# HOON : feature_map_2's input channel size is sames as feature_map_1's
# 마찬가지로 피쳐맵 1번과 2번의 인풋 성질이 비슷하여 크게 바꿀 것이 없었다
# 하지만 다른 각도로 피쳐맵을 추가, 이를 정규화 할 필요가 있다면 아래의 클래스를 
# 다르게 변형해야한다
...
# HOONING : overlap, nms_thesh 0.9
def nm_suppression(boxes, scores, overlap=0.45, top_k=200):   #0.45, 200
    """
    Non-Maximum Suppression을 실시하는 함수.
...
class SSD(nn.Module):
    ...
    def forward(self, x):
        ...
        sources_tmp = list()  # loc와 conf로의 입력 source1-6을 저장
        ...
        # vgg의 conv5_3까지 계산한다
        # 중간계층에 피쳐맵이 추가된다. 
        # 추가된 피쳐맵은 정규화 계층을 거치고 다음 단계 (loc&conf)로 넘어간다
        for k in range(23,30):   #HOON
            x = self.vgg[k](x)
