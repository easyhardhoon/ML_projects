...
def make_extras_X():
    ...
    #cfg = [256, 512, 128, 256, 128, 256, 128, 256]
    cfg = [1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024]
    # layers += [conv2d, nn.leaky_reLU(inplace=True)]
    ...
    layers += [nn.Conv2d(in_channels, cfg[0], kernel_size=(1)), bn1, nn.LeakyReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[0], cfg[1], kernel_size=(3), stride=2, padding=1), bn2, nn.LeakyReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[1], cfg[2], kernel_size=(1)), bn3, nn.LeakyReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[2], cfg[3], kernel_size=(3), stride=2, padding=1), bn4, nn.LeakyReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[3], cfg[4], kernel_size=(1)), bn5, nn.LeakyReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[4], cfg[5], kernel_size=(3)), bn6, nn.LeakyReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[5], cfg[6], kernel_size=(1)), bn7, nn.LeakyReLU(inplace=True)]
    layers += [nn.Conv2d(cfg[6], cfg[7], kernel_size=(3)), bn8, nn.LeakyReLU(inplace=True)]
...
def make_loc_conf(num_classes=21, bbox_aspect_num=[4, 6, 6, 6, 6]):
...
class Detect(Function):
    ...
    # below "conf_thresh" is main point. 0.01 / 0.1/ 0.5 / 0.6 
    # NOTE using mAP's algorithm . make vanilla B_box's making mechanism more comple
    @staticmethod
