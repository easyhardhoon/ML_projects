...
def make_extras():
    ...
    cfg = [1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024]
...
def make_loc_conf(num_classes=21, bbox_aspect_num=[6,6,6,6,6,6,6,6,6]):
    ...
    loc_layers += [nn.Conv2d(1024, bbox_aspect_num[7]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(1024, bbox_aspect_num[7]
                              * num_classes, kernel_size=3, padding=1)]

    # extra(source7)에 대한 합성곱층
    loc_layers += [nn.Conv2d(1024, bbox_aspect_num[8]
                             * 4, kernel_size=3, padding=1)]
    conf_layers += [nn.Conv2d(1024, bbox_aspect_num[8]
                              * num_classes, kernel_size=3, padding=1)]
...
def nm_suppression(boxes, scores, overlap=0.5, top_k=200):   #0.45, 200
    """
    Non-Maximum Suppression을 실시하는 함수.
...
class Detect(Function):
    ...
    @staticmethod
    def forward(self, loc_data, conf_data, dbox_list, conf_thresh=0.015, top_k=200, nms_thresh=0.5): #현 pytorch version에 맞게 수정함
        """
        순전파 계산을 수행한다.
