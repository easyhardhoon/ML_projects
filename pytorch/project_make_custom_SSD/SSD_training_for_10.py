...
from utils.ssd_model_for_10 import SSD
from utils.ssd_model_for_10 import make_datapath_list, VOCDataset, DataTransform, Anno_xml2list, od_collate_fn
from utils.ssd_model_for_10 import MultiBoxLoss
...
#HOON
#문제에서 주어진 조건대로 조건을 수정하였다.
#클래스 형태로 되어있기 때문에 아래의 조건을 수정하면 그에 맞게
#SSD 클래스가 새롭게 정의된다.
#이러한 조건을 이용하여 더욱 다양하게 모델을 수정할 수 있다.
#
# SSD300 설정

# NOTE
# num_classes , bbox_aspect_num : initiazize & use in SGD class
# input_Size,feature_maps,steps,min_Sizes,max_sizes,aspect_ratios : initialize & use in DBOX class
# In aspect ratios, extra prior is maybe "1:1 min_size" & "1:1 max_size" 
# (1) : min_sizes, max_sizes, aspect_ratios, steps  control D_box's size spec
# (2) : bbox_aspect_num control D_box's num per pixel

# NOTE  #just change "bbox_aspect_num" -> [6.6.6.6.6.6.6.] & [nms_thresh, conf_thresh, top_k]

ssd_cfg = {
    'num_classes': 21,  # 배경 클래스를 포함한 총 클래스 수
    'input_size': 300,  # 이미지의 입력 크기
    'bbox_aspect_num': [6,6,6, 6,6, 6, 6, 6, 6],  # 출력할 DBox의 화면비의 종류
    'feature_maps': [38, 19,19,19, 19, 10, 5, 3, 1],  # 각 source의 이미지 크기
    'steps': [8, 16,16,16, 16, 32, 64, 100, 300],
    'min_sizes': [30, 60,60,60, 60, 111, 162, 213, 264],  # DBOX의 크기(최소)
    'max_sizes': [60, 111,111, 111,111,162, 213, 264, 315],  # DBOX의 크기(최대)
    'aspect_ratios': [[2,3], [2,3],[2,3],[2,3],[2, 3], [2, 3], [2, 3], [2,3], [2,3]],
}
...
start_epoch = 10 
weight_file = f'./weights/12_ssd300_{start_epoch}.pth'
...
def train_model(net, dataloaders_dict, criterion, optimizer, start_epoch, end_epoch):
    ...
    for epoch in range(start_epoch,end_epoch+1):
        ...
        for phase in ['train', 'val']:
            ...
            for images, targets in iterator:
                ...
                if phase == 'train':
                    ...
                    loss = loss_l + loss_c
                    #print("localization loss : {}".format(loss_l))
                    #print("classification loss : {}".format(loss_c))
        ...
        # 네트워크를 저장한다
        if ((epoch+1) % 10 == 0):
            torch.save(net.state_dict(), 'weights/12_ssd300_' +
                       str(epoch+1) + '.pth')
